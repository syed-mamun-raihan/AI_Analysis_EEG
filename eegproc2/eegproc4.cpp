 ...
 *    sraihan@uwo.ca
 *    Version 1.4					   		   *
 *   Fixed Length and Order Version Test: Ar and Cepstrum coefficients	   *
 ***************************************************************************/
...
//[B,A]=butter(6,[1/500,32/250],'bandpass');
	...
	RawEEGData::p_DebugWrite(const double *a_data,
				...
				const long& a_length,
				const std::char_traits<char> mode)const
	{
		ofstream m_dbgFile(a_fileName, mode);
			...
			this->p_Read (l_data, l_marker) == FRAME_LENGTH)
	{
		...
		if (second == (m_args.m_noOfsamples -1) )
		{
			...
			this->p_Write(l_slopeCeps,l_slopeSpec, l_slopeRA,
					l_slopeSFR,NULL,NULL,
					l_avgFPE, NULL, NULL, l_marker);
			second = 0;
		} // if
		else
...
//		double l_ra_base = 0.0;
					...
					double& a_perform)const
	{
		...
		a_order
		= this->
		p_ArBurg(a_data, a_wgn, a_AR, a_perform, a_Ep, a_length);
