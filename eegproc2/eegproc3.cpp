 ...
 *    sraihan@uwo.ca							   *
 *   Fixed Length and Fixed Order Version				   *
 ***************************************************************************/
...
#define EXEC_DEBUG_PRINT(data,length,file) \
		...
		{\
			...
			this->p_DebugWrite(data,file,length, ios::ate);\
		}

#define FINALIZE_DEBUG_PRINT() firstTime = false;

#define EXEC_PRINT(data,length,file) \
		this->p_DebugWrite(data,file,length, ios::app);
...
//[B,A]=butter(6,[1/500,32/250],'bandpass');
			...
			const char a_marker[FRAME_LENGTH][2][DATA_READ_LEN])
	{
			...
			i < NO_MONTAGES;i++)
		{
			...
			this->m_meanOrder += MIN_ARORDER;
			this->m_meanLength += FRAME_LENGTH;
		}
		m_timestampFile.flush();
	}
	...
	*/
	...
	{
		ofstream m_dbgFile(a_fileName,std::_Ios_Openmode(mode));
...
RawEEGData::Process(void)
{
			...
			this->p_Read (l_data, l_marker) == FRAME_LENGTH)
	{
			...
			i < NO_MONTAGES;i++)
		{
			...
			long order = MIN_ARORDER;
			double length=FRAME_LENGTH;
			// Interface has not changed instead I process
			// with fixed order and length
			ceps[i][second] =
			this->
		...
		if (second == (m_args.m_noOfsamples -1) )
		{
			...
			EXEC_PRINT(l_slopeRA, NO_MONTAGES, APLHAOUT_TXT)
			EXEC_PRINT(l_slopeSFR, NO_MONTAGES, SFROUT_TXT)

			this->p_Write(l_avgFPE, NULL, NULL, l_marker);
			second = 0;
		} // if
		else
				...
				const long& a_noelements) const
	{
		...
		for(long i=0; i < a_noelements+1; i++)
			fittedFrame[i] = 0;
		...
		long	index = 1;
		for (long i = 1; i < FRAME_LENGTH; i++)
		{
			fittedFrame[index] = a_data[i];
			// this saves 2.1% processing time
			const long lastFrame = (index  == a_noelements)
						 ? 0: (index + 1);
			a_data[i] = a_data[i-1] +
				(fittedFrame[index] -
...
//		double l_ra_base = 0.0;
				...
				const long& a_length)const
	{
		...
		/* Keep the arrays and plans around from
		last time, since this
		is a very common case.
		Reallocate them if they change. */
		INIT_DEBUG_PRINT()
		if (firstTime) {
			/* Get any accumulated wisdom. */
			wisdom = fopen("wisfile", "r");
	...
	RawEEGData::p_SumOfCepstrums (const double a_data[FRAME_LENGTH],
					...
					double& a_perform)const
	{
		// ArBurg, OptimumAR need to be syncronized without
		// Number of parameter or type
		a_order
		= this->
		p_ArBurg(a_data, a_wgn, a_AR, a_perform, a_Ep, (long)a_length);
		a_length = a_length*double(SAMPLE_PERIOD)/double(FRAME_LENGTH);
	...
	// MAX_ARORDER is a const set to 40
	// fpeData is the variance
	const long
...
//		const double init_Eo = Eo;
		long	lowestOrder = MIN_ARORDER;

		double	den = 0.0 ;

		for (long m = 1; m <= MIN_ARORDER; m++)
		{
			Ao[0] = a_Ar[0] = 1;
