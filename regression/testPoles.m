function [success, dataX, dataY] = testPoles(sample1,sample2,classA,classB)
...
pack;
msg_out = ['Testing Poles Model Using NN, SVM, GLM(Linear-MLM), MR(Linear-LS) Discriminant Analysis' ]
sampleStart = 1;
sampleLength = 1000;
...
trainOrTest = 0;
    ...
    for i=0:sampleLength:(sampleSize-sampleLength+1)
        ...
        Ao = arburg(test1,ar_order);
        Rx = roots(Ao);
        dataA = [];
        for j=1:1:ar_order
            [rho] = abs(Rx(j)); 
            [theta] = angle(Rx(j)); 
            dataA = [dataA rho theta];
        end
        
        Ao = arburg(test2,ar_order);
        Rx = roots(Ao);
        dataB = [];
        for j=1:1:ar_order
            [rho] = abs(Rx(j)); 
            [theta] = angle(Rx(j)); 
            dataB = [dataB rho theta];
        end
        dataA = dataA';
        dataB = dataB';
        if trainOrTest == 0 
